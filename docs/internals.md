...
| the strict decoders' **unknown-name list as shared scratch** (`odjsonrt.UnknownNames`) instead of an `[8][]byte` zeroed on entry to every object, and the matched name no longer stored for an error path that can read it back | `json/v2` small decode **-3.5%** and `encoding/json` small **-3.2%** (both p=0.000); twitter within noise. Frames: 416 → 192 bytes for the three-field `Author`, 464 → 224 for `User` |
