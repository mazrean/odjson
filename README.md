...
JSON encoding and decoding that already goes through the standard `encoding/json/v2` gets 2.8×–4.2× faster by adding the comment below and running `go generate`.
```go
...
<picture>
  ...
  <img alt="Time per operation, lower is better. Marshal large: encoding/json/v2 395 µs, with odjson 94 µs, sonic 116 µs, go-json 242 µs. Marshal small: 1037 ns, with odjson 266 ns, sonic 311 ns, go-json 404 ns. Unmarshal large: 1078 µs, with odjson 380 µs, sonic 508 µs, go-json 656 µs. Unmarshal small: 1856 ns, with odjson 530 ns, sonic 990 ns, go-json 784 ns." src="./docs/assets/bench-light.svg" width="912">
</picture>

The benchmarks measure **2.8×–4.2× over `encoding/json/v2`** on encode and decode alike. They also put odjson ahead of [`goccy/go-json`](https://github.com/goccy/go-json) on every measurement, by 1.5×–2.6×, and ahead of [`bytedance/sonic`](https://github.com/bytedance/sonic) — which JIT-compiles hand-written assembly and uses SIMD — on all four: 1.34× and 1.87× on the two decodes, 1.23× on the large encode and 1.17× on the small one. The encode margins were level with sonic until September 2026, when the string writer stopped scanning and copying in two passes; see [docs/internals.md](./docs/internals.md).
...
`encoding/json` benefits too, since 1.27 implements it on top of `encoding/json/v2` internally, but odjson is tuned to get the most out of `encoding/json/v2`, which is the recommended way to use it. Through `encoding/json` the two encodes come out at 3.6× and the two decodes at 1.3×–1.6×: the encodes take the direct write into the internal buffer described below, while the decodes go through the public API, because the coder flags `encoding/json` sets allow input that the direct path's strict parsers refuse.
...
- writing a string's bytes as the scan judges them, so that no string pays a
  separate copy
- writing integer digits straight into the output buffer instead of formatting
  them into a scratch array first
