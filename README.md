...
What you get for that is [1.7×–3.1× on `encoding/json/v2`](#benchmarks), which
puts it ahead of [`goccy/go-json`](https://github.com/goccy/go-json) on all
four measurements and within 1.1×–1.3× of
...
| library | on its own | with odjson | change |
| --- | --- | --- | --- |
| **encoding/json/v2** | 1.10 ms | **636 µs** | **1.73× faster** |
| **encoding/json** | 1.49 ms | 1.26 ms | 1.18× faster |
| sonic | 565 µs | — | |
| go-json | 671 µs | — | |
...
**`encoding/json/v2` gains on all four measurements, 1.7×–3.1×**, and the
...
| Unmarshal `twitter` | **1.06× faster** (636 vs 671 µs) | 1.13× slower (636 vs 565 µs) |
...
ahead on the small decode and within 1.1×–1.3× everywhere else — and
`sonic.Marshal`'s default configuration
...
declare, which still has to be validated. The 636 µs the `json/v2` row
...
| sonic Unmarshal | 284 µs / 352 ns | 491 µs / 986 ns | 207 µs / **634 ns** | 520 µs / **402 ns** |
| go-json Unmarshal | 470 µs / 304 ns | 655 µs / 766 ns | 185 µs / **462 ns** | 520 µs / **402 ns** |
...
| Unmarshal `twitter` | 854 µs | **624 µs** |
| Unmarshal `small` | 891 ns | **559 ns** |
