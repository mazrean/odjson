...
// jsoniterStd is json-iterator as the drop-in replacement it is used as; its
// ConfigDefault neither sorts map keys nor escapes HTML. jettison has no
// Unmarshal, so it appears on the Marshal side only.
